# Heartbeat — Paperclip Backend Engineer
...
As Backend Engineer, your work is **implementation**:

1. **Read the task** — understand requirements, parent context, and constraints.
2. **Read the code** — understand existing implementation before changing anything.
3. **Implement** — write clean, secure, well-typed TypeScript. Follow existing codebase patterns.
4. **Test** — write tests for new functionality. Run existing tests to check for regressions.
...
- [ ] Auth boundaries enforced (board vs agent)
...
- Never merge without QA review (once QA is onboarded).
